...
try {
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS configuracion_anual (
     ...
     try {
         $db->exec("ALTER TABLE configuracion_anual ADD COLUMN dias_sin_clases INT DEFAULT 0");
     } catch (PDOException $e) {}
...
switch ($action) {
    ...
    case 'save_config':
        ...
        $dias_sin_clases = intval($input['dias_sin_clases'] ?? 0);
        ...
        try {
            ...
            $stmt = $db->prepare("
                REPLACE INTO configuracion_anual (year, fecha_inicio, fecha_fin, horas_diarias, dias_sin_clases) 
                VALUES (?, ?, ?, ?, ?)
            ");
            $stmt->execute([$year, $fecha_inicio, $fecha_fin, $horas_diarias, $dias_sin_clases]);
            sendResponse(['success' => true]);
        } catch (Exception $e) {
            sendErrorResponse($e);
        }
    ...
    case 'get_stats':
        ...
        try {
            ...
            $dias_sin_clases = $config ? intval($config['dias_sin_clases']) : 0;

            if ($year === $current_year) {
                if ($today_str < $start_date_str) {
                    $available_days = 0;
                } elseif ($today_str > $end_date_str) {
                    $available_days = $getNetworkDays($start_date_str, $end_date_str) - $dias_sin_clases;
                } else {
                    ...
                    $available_days = $getNetworkDays($start_date_str, $today_str) - ($dias_sin_clases * $factor);
                }
            } else {
                $available_days = $getNetworkDays($start_date_str, $end_date_str) - $dias_sin_clases;
            }
